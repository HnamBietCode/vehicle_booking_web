...
-- Feature 4: Expand notification types and ref_types
ALTER TABLE notifications MODIFY COLUMN type ENUM('BOOKING_ASSIGNED','BOOKING_ACCEPTED','TRIP_COMPLETED','TRIP_STARTED','PAYMENT_DONE','DRIVER_APPROVED','DRIVER_REJECTED','RENTAL_CREATED','RENTAL_ACCEPTED','RENTAL_REJECTED','RENTAL_CANCELLED','RENTAL_PAID','SOBER_CREATED','SOBER_ACCEPTED','SOBER_ARRIVED','SOBER_STARTED','SOBER_COMPLETED','SOBER_CANCELLED','SOBER_PAID','DEPOSIT_SUCCESS','WITHDRAW_REQUEST','WITHDRAW_APPROVED','WITHDRAW_REJECTED','GENERAL') NOT NULL;

ALTER TABLE notifications MODIFY COLUMN ref_type ENUM('RENTAL','BOOKING','SOBER','DRIVER','WALLET','SYSTEM') NULL;

-- Feature 5: Admin reply for ratings
SET @s = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ratings' AND COLUMN_NAME = 'admin_comment' AND TABLE_SCHEMA = DATABASE()) = 0, 'ALTER TABLE ratings ADD COLUMN admin_comment TEXT AFTER comment', 'SELECT 1'));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @s = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ratings' AND COLUMN_NAME = 'replied_at' AND TABLE_SCHEMA = DATABASE()) = 0, 'ALTER TABLE ratings ADD COLUMN replied_at DATETIME AFTER admin_comment', 'SELECT 1'));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;
ALTER TABLE notifications MODIFY COLUMN type VARCHAR(50) NOT NULL;
ALTER TABLE notifications MODIFY COLUMN ref_type VARCHAR(50) NULL;
